...
async function createSemesterSubject(data) {
    ...
    // ✅ FIXED: Convert to strings instead of parseInt
    ...
    const existingAssociation = await prisma.semesterSubject.findUnique({
        where: {
            semesterId_subjectId: {
                semesterId: stringSemesterId, // ✅ FIXED: Use string IDs
                subjectId: stringSubjectId,   // ✅ FIXED: Use string IDs
            },
        },
    });
 ...
 * @param {string} associationId - The ID of the SemesterSubject association to delete.
 */
async function deleteSemesterSubject(associationId) {
    // ✅ FIXED: Convert associationId to string
    const stringAssociationId = String(associationId);

    try {
        await prisma.semesterSubject.delete({ where: { id: stringAssociationId } }); // ✅ FIXED: Use string ID
    } catch (error) {
