...
async function createSection(data) {
    ...
    // ✅ FIXED: Convert semesterId to string instead of parseInt
    ...
    const existingSection = await prisma.section.findFirst({
        where: {
            name,
            semesterId: stringSemesterId, // ✅ FIXED: Use string ID
        },
    });

    if (existingSection) {
        throw createError(409, `Section "${name}" already exists for semester ID ${stringSemesterId}.`);
    }
...
async function updateSection(sectionId, data) {
    ...
    // ✅ FIXED: Convert sectionId to string
    const stringSectionId = String(sectionId);

    const updateData = {};
    if (name !== undefined && name !== null) updateData.name = name;
    if (semesterId !== undefined && semesterId !== null) updateData.semesterId = String(semesterId); // ✅ FIXED: Convert to string

    const section = await prisma.section.findUnique({ where: { id: stringSectionId } }); // ✅ FIXED: Use string ID
    if (!section) {
        throw createError(404, 'Section not found.');
    }

    // Check for uniqueness during update if name or semesterId are changed
    if ((name !== undefined && name !== section.name) || (semesterId !== undefined && String(semesterId) !== section.semesterId)) {
        ...
        const existingConflict = await prisma.section.findFirst({
            where: {
                name: newName,
                semesterId: newSemesterId, // ✅ FIXED: Already a string from updateData
                NOT: { id: stringSectionId }, // ✅ FIXED: Use string ID
            },
        });
        if (existingConflict) {
    ...
    try {
        const updatedSection = await prisma.section.update({
            where: { id: stringSectionId }, // ✅ FIXED: Use string ID
            data: updateData,
            include: {
                semester: {
                    include: {
...
async function deleteSection(sectionId) {
    ...
    try {
        await prisma.section.delete({ where: { id: stringSectionId } }); // ✅ FIXED: Use string ID
    } catch (error) {
