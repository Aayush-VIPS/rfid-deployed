    ...
    try {
        const newSubject = await prisma.subject.create({
            data: {
                code,
                name,
                credits: credits ? parseInt(credits) : null, // ✅ CORRECT: credits is a numeric field, not an ObjectId
            },
        });
        return newSubject;
    } catch (error) {
...
async function updateSubject(subjectId, data) {
    ...
    // ✅ FIXED: Convert subjectId to string
    const stringSubjectId = String(subjectId);

    const updateData = {};
    if (code) updateData.code = code;
    if (name) updateData.name = name;
    if (credits !== undefined) updateData.credits = credits ? parseInt(credits) : null; // ✅ CORRECT: credits is a numeric field, not an ObjectId

    try {
        const updatedSubject = await prisma.subject.update({
            where: { id: stringSubjectId }, // ✅ FIXED: Use string subjectId
            data: updateData,
        });
...
async function deleteSubject(subjectId) {
    ...
    try {
        await prisma.subject.delete({ where: { id: stringSubjectId } }); // ✅ FIXED: Use string subjectId
    } catch (error) {
