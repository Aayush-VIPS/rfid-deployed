    ...
    try {
        ...
        const newFaculty = await prisma.$transaction(async (tx) => {
            ...
            const faculty = await tx.faculty.create({
                data: {
                    userId: user.id, // ✅ This is already correct - user.id from create is a string
                    name,
                    empId,
                    phone,
                    rfidUid,
...
async function updateFaculty(facultyId, data) {
    ...
    const faculty = await prisma.faculty.findUnique({ where: { id: stringFacultyId } });
    if (!faculty) {
    ...
    if (empId) {
        const existingEmpId = await prisma.faculty.findUnique({ where: { empId } });
        if (existingEmpId && existingEmpId.id !== stringFacultyId) { // ✅ FIXED: Use stringFacultyId
    ...
    if (rfidUid) {
        const existingRfid = await prisma.faculty.findUnique({ where: { rfidUid } });
        if (existingRfid && existingRfid.id !== stringFacultyId) { // ✅ FIXED: Use stringFacultyId
    ...
    try {
        const updatedFaculty = await prisma.$transaction(async (tx) => {
            ...
            await tx.user.update({
                where: { id: faculty.userId }, // ✅ This is correct - faculty.userId is already a string
                data: {
            ...
            return tx.faculty.update({
                where: { id: stringFacultyId }, // ✅ FIXED: Use stringFacultyId
                data: {
                    name: name || undefined,
...
async function deleteFaculty(facultyId) {
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            await tx.faculty.delete({ where: { id: stringFacultyId } }); // ✅ FIXED: Use stringFacultyId
            await tx.user.delete({ where: { id: faculty.userId } }); // ✅ This is correct - faculty.userId is already a string
        });
    } catch (error) {
 ...
 * @param {string} subjectInstanceId - The ID of the SubjectInstance.
...
async function assignFacultyToSubjectInstance(subjectInstanceId, facultyId) {
    // ✅ FIXED: Convert both IDs to strings
    const stringSubjectInstanceId = String(subjectInstanceId);
    const stringFacultyId = String(facultyId);

    const subjectInstance = await prisma.subjectInstance.findUnique({
        where: { id: stringSubjectInstanceId }
    });
    ...
    try {
        const updatedSubjectInstance = await prisma.subjectInstance.update({
            where: { id: stringSubjectInstanceId }, // ✅ FIXED: Use stringSubjectInstanceId
            data: { faculty: { connect: { id: stringFacultyId } } }, // ✅ FIXED: Use stringFacultyId
        });
