...
async function createScheduledClass(data) {
  ...
  // ✅ FIXED: Convert subjectInstanceId to string instead of parseInt
  ...
  // ✅ FIXED: Keep facultyId as string instead of parseInt
  const derivedFacultyId = facultyId ? String(facultyId) : subjectInstance.facultyId;
  ...
  try {
    ...
    const newScheduledClass = await prisma.scheduledClass.create({
      data: {
        ...
        subjectInst: { connect: { id: stringSubjectInstanceId } }, // ✅ FIXED: Use string ID
      },
      include: {
        subject: true,
        section: {
...
async function updateScheduledClass(id, data) {
  ...
  // ✅ FIXED: Convert subjectInstanceId to string instead of parseInt
  const stringSubjectInstanceId = subjectInstanceId ? String(subjectInstanceId) : undefined;
  ...
  const existingScheduledClass = await prisma.scheduledClass.findUnique({
    where: { id: stringId }, // ✅ FIXED: Use string ID
    include: {
      subjectInst: {
        select: { subjectId: true, sectionId: true, facultyId: true }
      }
  ...
  if (
    ...
    (stringSubjectInstanceId && stringSubjectInstanceId !== existingScheduledClass.subjectInstId)
  ) {
    const existingConflict = await prisma.scheduledClass.findFirst({
      where: {
        ...
        NOT: { id: stringId }, // ✅ FIXED: Use string ID
      },
    });
    if (existingConflict) {
  ...
  const updateData = {
    ...
    faculty: facultyId !== undefined
      ? (facultyId ? { connect: { id: String(facultyId) } } : { disconnect: true }) // ✅ FIXED: Use String()
      : undefined,
    subjectInst: stringSubjectInstanceId ? { connect: { id: stringSubjectInstanceId } } : undefined,
  };
