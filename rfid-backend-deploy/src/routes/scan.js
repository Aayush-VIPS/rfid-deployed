...
import WebSocket from 'ws'; // ✅ ADDED: Import WebSocket (was missing)
 ...
 * Body: { "rfidUid": "YOUR_RFID_UID", "sessionId": "mongodb_object_id" }
 */
router.post('/rfid', authenticateDeviceMiddleware, async (req, res, next) => {
    ...
    // ✅ FIXED: Add ObjectId validation for sessionId
    if (!ObjectId.isValid(sessionId)) {
        return next(createError(400, 'Invalid Session ID format.'));
    }

    // ✅ IMPROVED: Enhanced RFID UID validation
    if (typeof rfidUid !== 'string' || rfidUid.trim() === '') {
        return next(createError(400, 'RFID UID must be a valid non-empty string.'));
    }
    ...
    try {
        ...
        // ✅ NOTE: sessionId is already validated as ObjectId above and will be converted to string in the service
...
router.post('/enrollment-rfid', async (req, res, next) => {
    try {
        ...
        // ✅ IMPROVED: Enhanced RFID UID validation
        if (typeof rfidUid !== 'string' || rfidUid.trim() === '') {
            throw createError(400, 'RFID UID must be a valid non-empty string.');
        }
