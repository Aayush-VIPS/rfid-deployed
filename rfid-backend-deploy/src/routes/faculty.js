 ...
 * @access Private (ADMIN, PCOORD)
 * Body: { "subjectInstanceId": string, "facultyId": string }
 */
router.post('/assign-subject-instance', async (req, res, next) => {
    ...
    // ✅ FIXED: Add ObjectId validation for both IDs
    if (!ObjectId.isValid(subjectInstanceId) || !ObjectId.isValid(facultyId)) {
        return next(createError(400, 'Invalid SubjectInstance ID or Faculty ID format.'));
    }
