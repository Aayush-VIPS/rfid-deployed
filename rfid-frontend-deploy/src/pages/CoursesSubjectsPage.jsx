        ...
        <>
            ...
            <div className="management-section-card">
                ...
                <form onSubmit={(e) => {
                    ...
                    if (subjectId) {
                        // ✅ FIXED: Keep as string instead of parsing to integer
                        semesterSubjectHandlers.handleAddSubject(semester.id, subjectId);
...
function CoursesSubjectsPage() {
    ...
    const fetchCoursesAndDepartments = async () => {
        try {
            ...
            console.log('Departments count:', departmentsRes.data.length);
    ...
    const courseHandlers = {
        handleFormChange: (e) => {
            ...
            setCourseFormData(prev => {
                const newData = {
                    ...prev,
                    // ✅ FIXED: Keep departmentId as string for MongoDB ObjectId, only parse numeric fields
                    [name]: (name === 'durationYears' || name === 'totalSemesters') ? (value ? parseInt(value) : '') : value
                };
        ...
        handleFormSubmit: async (e) => {
            ...
            // ✅ IMPROVED: Add validation for departmentId
        ...
        handleEditClick: (course) => {
            ...
            setCourseFormData({
                name: course.name, 
                departmentId: course.departmentId, // ✅ Keep as string
    ...
    const semesterHandlers = {
        handleFormChange: (e) => {
            const { name, value } = e.target;
            // ✅ FIXED: Keep courseId as string, only parse numeric fields
            setSemesterFormData(prev => ({ ...prev, [name]: (name === 'number' || name === 'academicYear') ? (value ? parseInt(value) : '') : value }));
        },
        handleFormSubmit: async (e) => {
            e.preventDefault();
    ...
    const sectionHandlers = {
        handleFormChange: (e) => {
            const { name, value } = e.target;
            // ✅ FIXED: Keep semesterId as string
