...
import { getCurrentDateInIST, formatTimeInIST } from '../utils/timezone.js'; // ✅ ADDED: Import timezone utilities
...
function TeacherDashboard() {
  ...
  const [reportFilters, setReportFilters] = useState({
    subjectId: '',
    sectionId: '',
    date: getCurrentDateInIST(), // ✅ FIXED: Use IST date instead of UTC
  });
  ...
  const fetchTeacherClasses = async () => {
    ...
    try {
      ...
      // Store previous active session ID for comparison
      ...
      if (currentTeacherActiveSession) {
        ...
        if (previousActiveSessionId && activeSessionDetails) {
          toast.success(`Session for ${activeSessionDetails.subjectInst?.subject?.name || 'your class'} has been closed.`);
        }
  ...
  useEffect(() => {
    ...
    // Set up polling for real-time session updates (every 1 second)
    const pollingInterval = setInterval(() => {
      fetchTeacherClasses(); // Refresh teacher classes to get session updates
    }, 1000);
  ...
  // New function to handle manual session start without scheduled class
  const handleManualStartSession = async (subjectInstanceId) => {
    if (activeSessionId) {
      toast.error("You already have an active session. Please close it before starting a new one.");
      return;
    }
    try {
      // Call a different endpoint or modify the existing one to handle manual starts
      const response = await api.post('/api/session/start-manual', {
        facultyId: user.facultyId,
        subjectInstanceId: subjectInstanceId // Pass subject instance directly
      });
      const newSessionId = response.data.session.id;
      setActiveSessionId(newSessionId);
      setActiveSessionDetails(response.data.session);
      activeSessionToastShownRef.current = newSessionId;
      toast.success('Manual session started successfully! Redirecting to attendance board...');
      navigate(`/attendance-board/${newSessionId}`);
    } catch (error) {
      console.error('Error starting manual session:', error);
      toast.error(error.response?.data?.message || 'Failed to start manual session.');
    }
  };
      ...
      <div className="teacher-dashboard-container">
        ...
        {activeSessionDetails ? (
          <div className="active-session-banner active">
            ...
            <p>Started at: {formatTimeInIST(activeSessionDetails.startAt)}</p> {/* ✅ FIXED: Use IST formatting */}
          ...
          <div className="assigned-classes-grid">
            {teacherClasses.map((cls) => ( // cls is a SubjectInstance
              <div key={cls.id} className="class-card">
                ...
                {/* Manual Session Start - Always Available */}
                <div className="manual-session-section mt-3">
                    <h5>Start Class Now</h5>
                    <button
                        onClick={() => handleManualStartSession(cls.id)} // Start session for this subject instance
                        disabled={activeSessionId !== null}
                        className="record-button manual-start-button"
                    >
                        {activeSessionId ? 'Session Active' : 'Start Class Now'}
                    </button>
                    {/* <p className="manual-start-note">Start a class session without depending on timetable</p> */}
                </div>
